...
def login(request):
    if User.objects.filter(email=request.data.get('username')).exists():
        if User.objects.get(email=request.data.get('username')).password == request.data.get('password'):
            ...
            return Response({'loginSuccess': True, 'email': request.data.get('username'), 'nickname': user.nickname, 'address': user.address, 'spoon_cnt': user.spoon_cnt, 'token': token}, status=status.HTTP_200_OK)
